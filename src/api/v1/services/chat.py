                ...
                if line:
                    ...
                    if decoded_line.startswith("data:"):
                        try:
                            ...
                            flowise_data = json.loads(decoded_line.replace("data: ", "").strip())
                            text = flowise_data.get("text", "")
                            
                            if text:  # Only yield if there's actual content
                                if "object" in payload:  # Thrive format
                                    response = {
                                        "object": "message",
                                        "id": str(uuid.uuid4()),
...
async def handle_chat_completion(body: Dict[str, Any]) -> Generator[str, None, None]:
    try:
        ...
        # Pass through the original request format
        for chunk in fetch_flowise_stream(FLOWISE_PREDICTION_URL, body):
            yield chunk
